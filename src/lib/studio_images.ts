...
export type inspected_cos_object = { sha256: string; version_id?: string; studio_request_id?: string; etag?: string };
export interface cos_adapter { verify_versioning(): Promise<void>; inspect_object(object_key: string): Promise<inspected_cos_object | undefined>; upload_object(input: prepared_image, studio_request_id?: string): Promise<{ version_id: string }>; replace_object?(input: prepared_image, expected_etag: string, studio_request_id?: string): Promise<{ version_id: string }>; delete_object(object_key: string, version_id: string): Promise<void>; }
...
export const publish_prepared_images = async (images: readonly prepared_image[], adapter: cos_adapter, on_created?: (image: published_image) => Promise<void>, studio_request_id?: string): Promise<publish_result> => {
  ...
  try {
    ...
    try {
      const created = await adapter.upload_object(owned_image, studio_request_id);
...
export class tencent_cos_adapter implements cos_adapter {
  ...
  /** Reads digest, exact version, and Studio request ownership metadata for reconciliation. */
  async inspect_object(object_key: string): Promise<inspected_cos_object | undefined> { valid_adapter_key(object_key, this.config.root_prefix); try { const result = await this.client.headObject({ Bucket: this.config.bucket, Region: this.config.region, Key: object_key }); const value = result.headers?.['x-cos-meta-sha256']; const version_id = result.headers?.['x-cos-version-id']; const studio_request_id = result.headers?.['x-cos-meta-studio-request-id']; const etag = result.headers?.etag ?? result.ETag; if (typeof value !== 'string' || !/^[a-f0-9]{64}$/i.test(value)) throw new studio_image_error('missing_remote_digest', `COS object lacks a valid sha256 digest: ${object_key}`); if (version_id !== undefined && !version_id) throw new studio_image_error('missing_remote_digest', `COS object has an invalid version id: ${object_key}`); if (studio_request_id !== undefined && !/^(?:[a-f0-9]{32}|[a-f0-9]{64}|[a-f0-9]{8}-(?:[a-f0-9]{4}-){3}[a-f0-9]{12})$/.test(studio_request_id)) throw new studio_image_error('missing_remote_digest', `COS object has an invalid Studio request id: ${object_key}`); if (etag !== undefined && (typeof etag !== 'string' || !etag || etag.length > 512 || /[\x00-\x1f\x7f]/.test(etag))) throw new studio_image_error('missing_remote_digest', `COS object has an invalid ETag: ${object_key}`); return { sha256: value.toLowerCase(), ...(version_id === undefined ? {} : { version_id }), ...(studio_request_id === undefined ? {} : { studio_request_id }), ...(etag === undefined ? {} : { etag }) }; } catch (error: unknown) { if (typeof error === 'object' && error !== null && 'statusCode' in error && (error as { statusCode?: unknown }).statusCode === 404) return undefined; throw error; } }
  /** Uploads an immutable object with a request identifier that supports crash reconciliation. */
  async upload_object(input: prepared_image, studio_request_id?: string): Promise<{ version_id: string }> { valid_adapter_key(input.object_key, this.config.root_prefix); if (studio_request_id !== undefined && !/^(?:[a-f0-9]{32}|[a-f0-9]{64}|[a-f0-9]{8}-(?:[a-f0-9]{4}-){3}[a-f0-9]{12})$/.test(studio_request_id)) invalid('Invalid Studio request id.'); const bytes = new Uint8Array(input.bytes); if (sha256(bytes) !== input.sha256) throw new studio_image_error('untracked_create', `Image bytes changed: ${input.object_key}`); const result = await this.client.putObject({ Bucket: this.config.bucket, Region: this.config.region, Key: input.object_key, Headers: { 'If-None-Match': '*', 'x-cos-meta-sha256': input.sha256, ...(studio_request_id === undefined ? {} : { 'x-cos-meta-studio-request-id': studio_request_id }) }, Body: Buffer.from(bytes), ContentLength: bytes.byteLength, ContentType: input.content_type }); if (!result.VersionId) throw new studio_image_error('untracked_create', `COS create returned no version id: ${input.object_key}`); return { version_id: result.VersionId }; }
  /** Creates a new version only when the inspected current object still has the authorized ETag. */
  async replace_object(input: prepared_image, expected_etag: string, studio_request_id?: string): Promise<{ version_id: string }> { valid_adapter_key(input.object_key, this.config.root_prefix); if (!expected_etag || expected_etag.length > 512 || /[\x00-\x1f\x7f]/.test(expected_etag)) invalid('Invalid COS replacement ETag.'); if (studio_request_id !== undefined && !/^(?:[a-f0-9]{32}|[a-f0-9]{64}|[a-f0-9]{8}-(?:[a-f0-9]{4}-){3}[a-f0-9]{12})$/.test(studio_request_id)) invalid('Invalid Studio request id.'); const bytes = new Uint8Array(input.bytes); if (sha256(bytes) !== input.sha256) throw new studio_image_error('untracked_create', `Image bytes changed: ${input.object_key}`); const result = await this.client.putObject({ Bucket: this.config.bucket, Region: this.config.region, Key: input.object_key, Headers: { 'If-Match': expected_etag, 'x-cos-meta-sha256': input.sha256, ...(studio_request_id === undefined ? {} : { 'x-cos-meta-studio-request-id': studio_request_id }) }, Body: Buffer.from(bytes), ContentLength: bytes.byteLength, ContentType: input.content_type }); if (!result.VersionId) throw new studio_image_error('untracked_create', `COS replacement returned no version id: ${input.object_key}`); return { version_id: result.VersionId }; }
